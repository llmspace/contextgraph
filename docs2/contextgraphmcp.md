# Context Graph MCP Server - Complete Reference Guide

This comprehensive guide documents everything about the Context Graph MCP (Model Context Protocol) server - all 35 tools, 40+ JSON-RPC methods, the 13-embedder teleological system, the 5-layer bio-nervous architecture, and complete protocol details.

## Table of Contents

1. [Architecture Overview](#architecture-overview)
2. [The 13-Embedder Teleological System](#the-13-embedder-teleological-system)
3. [The 5-Layer Bio-Nervous System](#the-5-layer-bio-nervous-system)
4. [MCP Protocol & Transport](#mcp-protocol--transport)
5. [All 35 Tools Reference](#all-35-tools-reference)
6. [All JSON-RPC Methods Reference](#all-json-rpc-methods-reference)
...
10. [Johari Window Classification](#johari-window-classification)
11. [Global Workspace Theory (GWT)](#global-workspace-theory-gwt)
...
The Context Graph MCP server is a bio-inspired knowledge management system that implements:
...
The 13 embedders are organized into 6 functional groups:
...
Each embedder has a natural oscillation frequency for the Kuramoto synchronization:
...
Each memory has a 13-dimensional "purpose vector" representing the normalized activation across all embedders. This vector is used for:
- Goal alignment computation
- Drift detection
- Cross-correlation analysis
- Teleological search

### Cross-Correlations (78D)

The system computes pairwise correlations between all 13 embedders:
- Total pairs: C(13,2) = 78
- Used for synergy detection between embedding spaces
- Adaptive learning updates the synergy matrix based on retrieval feedback

---

## The 5-Layer Bio-Nervous System

The architecture is organized into 5 hierarchical layers inspired by biological nervous systems:
...
**Returns**: `fingerprintId`, UTL metrics (entropy, coherence, learning score)

#### 2. `store_memory`
Store a memory node directly without UTL processing.
...
| `minSimilarity` | number | No | 0.0 | Min similarity threshold |
| `modality` | enum | No | - | Filter by modality |
...
Get current system status with live UTL metrics.

**No parameters required.**

**Returns**: Phase, fingerprint count, UTL metrics (entropy, coherence, learning score, Johari quadrant, consolidation phase, suggested action), 5-layer status.

#### 5. `get_graph_manifest`
Get the 5-layer bio-nervous system architecture description.

**No parameters required.**

#### 6. `utl_status`
Query current UTL (Unified Theory of Learning) system state.
...
Get current consciousness state from GWT system.
...
**Returns**: Order parameter r [0,1], mean phase psi, 13 oscillator phases, natural frequencies, coupling strength K.

#### 9. `get_workspace_status`
Get Global Workspace status (Winner-Take-All selection).
...
Trigger WTA workspace broadcast with a specific memory.
...
#### 12. `adjust_coupling`
Adjust Kuramoto oscillator coupling strength K.
...
### Adaptive Threshold Calibration (ATC) Tools (3)

#### 13. `get_threshold_status`
Get current ATC threshold status.
...
**Requirements**: Activity < 0.15, GPU < 30%
...
| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `min_confidence` | number | No | 0.7 | Min confidence threshold [0,1] |
| `limit` | integer | No | 20 | Max shortcuts (1-100) |

**Returns**: Paths traversed 5+ times with 3+ hops qualifying for direct edge creation.
...
Adjust a specific neuromodulator level.

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `modulator` | enum | Yes | - | `dopamine`, `serotonin`, `noradrenaline` (ACh is read-only) |
...
Get steering feedback from Gardener, Curator, Assessor.
...
| `direction` | enum | No | "forward" | `forward`, `backward`, `bidirectional`, `bridge`, `abduction` |
...
Search across all 13 embedder dimensions with configurable strategy.

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `query_content` | string | No | - | Content to search for |
| `query_vector_id` | string | No | - | Existing vector ID to use as query |
| `strategy` | enum | No | "adaptive" | `cosine`, `euclidean`, `synergy_weighted`, `group_hierarchical`, `cross_correlation_dominant`, `tucker_compressed`, `adaptive` |
| `scope` | enum | No | "full" | `full`, `purpose_vector_only`, `cross_correlations_only`, `group_alignments_only` |
| `specific_groups` | array[enum] | No | - | `factual`, `temporal`, `causal`, `relational`, `qualitative`, `implementation` |
...
| `min_similarity` | number | No | 0.3 | Min similarity threshold |
| `max_results` | integer | No | 20 | Max results (1-1000) |
| `include_breakdown` | boolean | No | true | Include per-component breakdown |

#### 25. `compute_teleological_vector`
Compute a complete 13-embedder teleological vector.
...
#### 26. `fuse_embeddings`
Fuse embeddings using synergy matrix.
...
| `custom_weights` | array[number] | No | - | Custom weights [E1..E13] |
...
Update synergy matrix from retrieval feedback.
...
CRUD operations for task-specific profiles.
...
Bootstrap autonomous system from teleological embeddings.

| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `confidence_threshold` | number | No | 0.7 | Min confidence [0,1] |
| `max_candidates` | integer | No | 10 | Max candidates (1-100) |

**Note**: The system discovers purpose autonomously from stored teleological fingerprints. Manual North Star tools have been removed because single 1024D embeddings cannot be meaningfully compared to 13-embedder teleological arrays.

#### 30. `get_alignment_drift`
Get current alignment drift state.
...
| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `limit` | integer | No | 20 | Max candidates (1-1000) |
| `min_staleness_days` | integer | No | 30 | Min age for staleness |
| `min_alignment` | number | No | 0.4 | Below this = candidate |

**Pruning criteria**: Staleness, low alignment, redundancy, orphaned status.
...
| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `max_memories` | integer | No | 100 | Max to process (1-10000) |
| `strategy` | enum | No | "similarity" | `similarity`, `temporal`, `semantic` |
| `min_similarity` | number | No | 0.85 | Min similarity threshold |
...
| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `min_confidence` | number | No | 0.6 | Min confidence [0,1] |
| `max_goals` | integer | No | 5 | Max to discover (1-20) |
...
**Services monitored**: Bootstrap, drift detector, drift corrector, pruning, consolidation, sub-goal discovery.

---

## All JSON-RPC Methods Reference
...
| Method | Description |
|--------|-------------|
| `graph/connect` | Create edge between nodes |
| `graph/traverse` | Traverse graph relationships |

### UTL Operations

| Method | Description |
|--------|-------------|
| `utl/compute` | Compute UTL metrics |
| `utl/metrics` | Get UTL metrics |
...
| `johari/cross_space_analysis` | Cross-space analysis (blind spots, opportunities) |
...
### Context Graph Specific (-32001 to -32099)
...
| -32019 | TOOL_NOT_IMPLEMENTED | Tool not yet implemented (FAIL FAST) |
...
| -32021 | NORTH_STAR_NOT_CONFIGURED | DEPRECATED - system operates autonomously |
...
Predefined weight profiles for multi-embedding search:
...
Every MCP response includes a `X-Cognitive-Pulse` header with live UTL metrics:
...
Valid transitions depend on triggers:
...
The system discovers and maintains purpose autonomously:

### Services

1. **Bootstrap Service**: Discovers purpose from teleological fingerprints
...
- Manual: ONE vector (1024D)
- Teleological: 13 DIFFERENT embeddings with different dimensions
...
```yaml
# Pre-task: Inject relevant context
hooks:
  pre-task:
    - name: context-inject
      command: |
        mcp call context-graph search_graph '{"query": "$TASK", "topK": 5}'

# Post-edit: Store code changes
  post-edit:
    - name: context-store
      command: |
        mcp call context-graph inject_context '{
          "content": "$CHANGES",
          "rationale": "Edit to $FILE",
          "modality": "code"
        }'

# Session end: Dream consolidation
  session-end:
    - name: consolidate
      command: |
...
```
1. Store initial context → store_memory / inject_context
2. Bootstrap autonomous system → auto_bootstrap_north_star
3. Work with context:
   - search_graph for retrieval
   - inject_context for new information
   - Check _cognitive_pulse in every response
4. Monitor alignment → get_alignment_drift
5. Correct drift if needed → trigger_drift_correction
6. End session → trigger_dream, trigger_consolidation
```
