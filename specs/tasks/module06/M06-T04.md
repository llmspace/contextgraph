# M06-T04: Implement CandleEmbeddingProvider

```yaml
metadata:
  id: "M06-T04"
  title: "Implement CandleEmbeddingProvider with GPU Acceleration"
  module: "module-06"
  ...
  spec_refs:
    - "docs2/constitution.yaml:305-323"   # 12-model embedding
    - "docs2/constitution.yaml:108-122"   # Performance: single_embed <10ms, batch_embed_64 <50ms
    - "docs2/contextprd.md:189-208"       # E8 Graph MiniLM 384D
```
...
2. Uses MiniLM-L6-v2 (384D) for simpler/faster embeddings (constitution E8)
3. Can be used WITHOUT the full embeddings crate dependency
4. Provides an ALTERNATIVE to `EmbeddingProviderAdapter` for lightweight deployments

---

## Problem Statement

The current architecture requires `context-graph-mcp` → `context-graph-embeddings` → full 12-model pipeline just for basic embeddings. This task creates a lightweight Candle-based provider in `context-graph-core` using MiniLM-L6-v2 (384D) that:

1. Implements `context_graph_core::traits::EmbeddingProvider` directly
2. Provides sub-10ms single embeddings on GPU
3. Works without the full embedding pipeline
4. Serves as reference implementation for trait conformance
