...
fn edge_case_offset_beyond_results() {
    ...
    let result = db.get_nodes_by_tag("test-source", Some(10), 100).unwrap();
...
fn edge_case_limit_zero() {
    ...
    let result = db.get_nodes_by_tag("any", Some(0), 0).unwrap();
...
fn edge_case_limit_greater_than_results() {
    ...
    for _ in 0..3 {
        let node = create_node_with_tags(vec!["limited"]);
        db.store_node(&node).unwrap();
    }

    println!("BEFORE: 3 nodes stored, querying with limit=100");
    let result = db.get_nodes_by_tag("limited", Some(100), 0).unwrap();
...
fn evidence_index_consistency() {
    ...
    println!(
        "STORED: Node {} with tags=[evidence,verification], source=evidence-source",
        node_id
    );
    ...
    println!("RESULT: All indexes correctly contain the node");
}
