...
mod tests {
    ...
    fn test_synthetic_valid_input() {
        ...
        let input_action = synthetic_input
            .get("action_type")
            .unwrap()
            .as_str()
            .unwrap();
        ...
        assert!(
            valid_actions.contains(&input_action),
            "action_type 'hypothesize' should be valid"
        );
        ...
        let input_target_len = synthetic_input
            .get("target")
            .unwrap()
            .as_str()
            .unwrap()
            .len() as u64;
        assert!(
            input_target_len >= target_min,
            "target length {} should be >= {}",
            input_target_len,
            target_min
        );
        assert!(
            input_target_len <= target_max,
            "target length {} should be <= {}",
            input_target_len,
            target_max
        );
        ...
        assert!(
            input_conf >= conf_min && input_conf <= conf_max,
            "confidence {} should be in [{}, {}]",
            input_conf,
            conf_min,
            conf_max
        );
        ...
        let input_rat_len = synthetic_input
            .get("rationale")
            .unwrap()
            .as_str()
            .unwrap()
            .len() as u64;
        assert!(
            input_rat_len >= rat_min && input_rat_len <= rat_max,
            "rationale length {} should be in [{}, {}]",
            input_rat_len,
            rat_min,
            rat_max
        );
        ...
        let input_uncertainty = synthetic_input
            .get("context")
            .unwrap()
            ...
            .as_str()
            .unwrap();
        let valid_uncertainties: Vec<&str> = uncertainty_enum
            .iter()
            .map(|v| v.as_str().unwrap())
            .collect();
        assert!(
            valid_uncertainties.contains(&input_uncertainty),
    ...
    fn test_verify_epistemic_in_all_tools() {
        ...
        assert!(
            names.contains(&"epistemic_action"),
            "epistemic_action missing from tool list"
        );
    }
    ...
    fn test_all_action_type_values() {
        ...
        assert!(
            values.contains(&"hypothesize"),
            "hypothesize: tentative belief"
        );
