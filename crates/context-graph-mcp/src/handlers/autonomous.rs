...
use context_graph_core::autonomous::{
    ConsolidationConfig, ConsolidationService, DriftCorrector, DriftDetector, DriftSeverity,
    ExtendedPruningConfig, MemoryContent, MemoryId, MemoryMetadata, MemoryPair, PruningConfig,
...
impl Handlers {
    ...
    pub(super) async fn call_get_pruning_candidates(
        ...
        let johari_list = match self.teleological_store.list_all_johari(params.limit * 2).await {
            Ok(list) => list,
            Err(e) => {
                error!(
                    error = %e,
        ...
        for candidate in &limited_candidates {
            *by_reason.entry(format!("{:?}", candidate.reason)).or_insert(0) += 1;
        }
