...
async fn test_fsv_real_embeddings_memory_lifecycle() {
    ...
    let store_request = make_request("memory/store", 1, json!({"content": content, "importance": 0.95}));
    ...
    let fingerprint_id = store_data.get("fingerprintId").or_else(|| store_data.get("fingerprint_id"))
        .and_then(|v| v.as_str()).expect("Should have fingerprint_id");
    ...
    // 2. RETRIEVE
    let retrieve_request = make_request("memory/retrieve", 2, json!({"fingerprintId": fingerprint_id}));
    ...
    let retrieve_data = extract_mcp_tool_data(&retrieve_response.result.expect("Should have result"));
    let retrieved_id = retrieve_data.get("fingerprint").and_then(|fp| fp.get("id"))
        .and_then(|v| v.as_str()).expect("Should have fingerprint.id");
    ...
    let results = search_result.get("results").and_then(|v| v.as_array()).expect("Should have results");
    assert!(!results.is_empty(), "Should find at least one result");
    assert_eq!(results[0].get("fingerprintId").and_then(|v| v.as_str()), Some(fingerprint_id));

    // 4. DELETE
    let delete_request = make_request("memory/delete", 4, json!({"fingerprintId": fingerprint_id, "soft": false}));
    ...
    let verify_result = handlers.dispatch(verify_request).await.result.expect("Should have result");
    assert_eq!(verify_result.get("count").and_then(|v| v.as_u64()).unwrap_or(0), 0);
}
...
async fn test_fsv_real_embeddings_semantic_relevance() {
    ...
    for (i, (content, importance)) in contents.iter().enumerate() {
        let request = make_request("memory/store", (i + 1) as i64, json!({"content": content, "importance": importance}));
    ...
    if results.len() >= 2 {
        ...
        let is_programming = top_content.contains("programming") || top_content.contains("Rust")
            || top_content.contains("Python") || top_content.contains("Kubernetes");
        if is_programming {
...
async fn test_fsv_real_embeddings_purpose_alignment() {
    ...
    // Search by purpose
    let purpose_vector: Vec<f64> = vec![0.9, 0.2, 0.2, 0.2, 0.8, 0.1, 0.3, 0.3, 0.2, 0.3, 0.5, 0.1, 0.1];
    ...
    let results = purpose_result.get("results").and_then(|v| v.as_array()).expect("Should have results");
    if !results.is_empty() {
