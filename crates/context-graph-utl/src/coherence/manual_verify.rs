...
mod manual_verification {
    ...
    fn manual_test_perfect_coherence() {
        ...
        let nearest_cluster = vec![vec![-1.0, 0.0, 0.0, 0.0], vec![-0.9, 0.1, 0.0, 0.0]];
        ...
        assert!(
            coherence > 0.9,
            "Expected high coherence > 0.9, got {}",
            coherence
        );
    ...
    fn manual_test_zero_coherence() {
        ...
        assert!(
            coherence < 0.5,
            "Expected low coherence < 0.5, got {}",
            coherence
        );
    ...
    fn manual_test_formula_verification() {
        ...
        let same_cluster = vec![vec![0.5, 0.5, 0.0, 0.0], vec![0.6, 0.4, 0.0, 0.0]];
        let nearest_cluster = vec![vec![0.0, 0.0, 0.5, 0.5]];
        ...
        assert!(
            (0.0..=1.0).contains(&result.connectivity),
            "Connectivity out of range"
        );
        assert!(
            (0.0..=1.0).contains(&result.cluster_fit),
            "ClusterFit out of range"
        );
        assert!(
            (0.0..=1.0).contains(&result.consistency),
            "Consistency out of range"
        );
    ...
    fn manual_test_weight_adjustment() {
        ...
        println!(
            "  Default weights (0.4, 0.4, 0.2): {:.4}",
            coherence_default
        );

        // Change to emphasize connectivity
        tracker.set_weights(0.8, 0.1, 0.1);
        let coherence_connectivity =
            tracker.compute_coherence(&vertex, connectivity, &cluster_context);
        println!(
            "  Connectivity emphasis (0.8, 0.1, 0.1): {:.4}",
            coherence_connectivity
        );

        // Change to emphasize cluster fit
        tracker.set_weights(0.1, 0.8, 0.1);
        let coherence_clusterfit =
            tracker.compute_coherence(&vertex, connectivity, &cluster_context);
        println!(
            "  ClusterFit emphasis (0.1, 0.8, 0.1): {:.4}",
            coherence_clusterfit
        );

        // Verify different weights produce different results
        assert!(
            (coherence_default - coherence_connectivity).abs() > 0.01
    ...
    fn manual_test_nan_inf_handling() {
        ...
        assert!(
            (0.0..=1.0).contains(&coherence_nan),
            "Output should be in [0, 1]"
        );
        println!(
            "  NaN connectivity -> {:.4} (using fallback 0.5)",
            coherence_nan
        );
        ...
        assert!(
            (0.0..=1.0).contains(&coherence_inf),
            "Output should be in [0, 1]"
        );
        println!(
            "  Inf connectivity -> {:.4} (using fallback 0.5)",
            coherence_inf
        );
